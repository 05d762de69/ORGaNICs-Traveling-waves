...
def attention_rivalry_model_ORGaNICs(
    ...
    for lay in [0, 3]:
        ...
        #Update response b
        new_results = np.zeros_like(results[:, 3, 12, :, t-1])
        for p in range(results.shape[0]):
            new_results[p] = results[p, 3, 12, :, int(max(t-1 - ((distance_matrix[idx][p] / propagation_speed) / dt), 0))]

        results[idx, 3, lay + 2, :, t] = results[idx, 3, lay + 2, :, t-1] +  (-results[idx, 3, lay + 2, :, t-1] + (1 + new_results.T.dot(wa[idx]))) * (dt/(tau_attention)) 
...
if is_windows:
    ...
    def run_simulation(
        nt, n_mass, dt, ntheta, n_m, n, sigma, m, wh,
        tau_v1, tau_attention, tau_adaptation, tau_opponency, tau_recurrence, tau_binocular, sigma_a, aKernel, results,
        wo, wa, input, wc, wr, wr_local, W_rr, b_0, c_attention, distance_matrix, ms_per_neuron):

        for t in range(1, nt):
            for idx in prange(n_mass):
                attention_rivalry_model_ORGaNICs(t, idx, dt, ntheta, n_m, n, sigma, m, wh,
                                        tau_v1, tau_attention, tau_adaptation, tau_opponency, tau_recurrence, tau_binocular, sigma_a, aKernel, results,
                                        wo, wa, input, wc, wr, wr_local, W_rr, c_attention, distance_matrix, ms_per_neuron)
